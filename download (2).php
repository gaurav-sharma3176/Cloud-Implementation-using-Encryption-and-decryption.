...
if(!isset($_POST['user'])){			//check if the user has given the user details if not then it prints you can't access this file
	...
	exit;
} elseif($_POST['user']=='username'&&$_POST['pass']=='pass') { // if user has given username = username and password = pass the its correct and link will be provided
	echo "<div style=\"font-family: 'Agency FB'\">Login<span>Successful</span></div></div><br>
	<div class=\"login\">
				<input type='button' value='Download File' onclick= echo "not yet completed">;><br>
				<input type='button' value='Merge File'  onclick=\"window.location='mergethisfile.php'\";><br>
				<input type='button' value='Split File'  onclick=\"window.location='splitthisfile.php'\";>
		</div>";
